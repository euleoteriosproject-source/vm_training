...
export default async function WorkoutDayPage({
  ...
  const { data: day } = await supabase
    .from("workout_days")
    .select(
      "id,name,estimated_minutes,workout_plan:workout_plans(status),workout_day_exercises(id,position,target_sets,rep_min,rep_max,rest_seconds,exercise:exercises(name_pt,primary_muscles,secondary_muscles,execution_instructions,breathing_instruction,common_errors,exercise_equipment(equipment(name)),exercise_media(storage_path,poster_path,status,media_type,media_role,execution_quality,is_primary,sort_order,author,source_name,source_url,license_code,license_url,attribution_text)))",
    )
    .eq("id", id)
    .maybeSingle();
  if (!day) notFound();
        ...
        {prepared.map((item, index) => (
          <ExerciseDetails
            ...
            trigger={
              ...
              >
                <Card className="grid overflow-hidden sm:grid-cols-[140px_1fr]">
                  <ViewportVideo
                  ...
                  <div className="flex items-center gap-4 p-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted">
                        {item.target_sets} séries · {item.rep_min}–
