-- VM Training v2.1: runtime substitutions may only select a published,
-- media-ready exercise that respects equipment, explicit avoids and active
-- movement-attention constraints. The mutation and undo contract remains
-- atomic and keeps the least-privilege public wrapper from v1.7.1.
...
begin
  ...
  from candidates
  order by rank_score desc, name_pt, id
  limit 1;

  if replacement_id is null then
    raise exception 'Nenhuma substituição compatível e com mídia disponível';
  end if;
...
comment on function private.substitute_workout_exercise_internal(uuid,text,uuid,uuid[])
  is 'Atomic v2.1 substitution restricted to published media-ready and user-compatible exercises.';
